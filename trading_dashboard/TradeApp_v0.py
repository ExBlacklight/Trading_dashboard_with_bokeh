...
def update():
    ...
    try:
        ...
        if volume_minus_one == None:
            ...
            cvol = pvol + (volume - volume_minus_one)
...
p.vbar(x='x', width=0.5, top='open', bottom='close', fill_color="color", line_color="color", source=source)
...
p.vbar(x='x', width=0.5, top='open', bottom='close', fill_color="color", line_color="color", source=source2)

menu = [(i,i) for ind,i in enumerate(kt.tokens.keys())]

dropdown = Dropdown(label="all Strikes", button_type="warning", menu=menu, width=320)
dropdown.on_click(handler)

buy_button = Button(label="BUY", button_type="success")
sell_button = Button(label="SELL", button_type="success")

buy_button.on_click(buy_button_handler)
sell_button.on_click(sell_button_handler)

radio_button_group = RadioButtonGroup(labels=radio_labels, active=0)
...
profit_text= Div(text=f'Profit = {np.round(profit,decimals=2)}')
rem_details_text = Div(text=f'<b>current_price</b> = None  <b>current_qty</b> = None')
rem_details_text_2 = Div(text=f'<b>current_trade_price</b> = None <b>open positions</b> = None ')
details_area = row(children=[profit_text])
...
layout = column(children=[dropdown,graph_area])
